  ...
  return (
    ...
    <h1 className='text-green-600 text-xl font-bold '>Logo</h1>
        ...
        <nav className="flex flex-row justify-between items-center space-x-14">
           ...
           (
            <div key={id}>
                <Link href={navlink.path} className={`hover:font-bold text-sm 4 ${pathName===navlink.path ? 'active' : ""} hover:text-green-500 hover:font-bold hover:text-md`}>{navlink.name}</Link>
            </div>
           ))}
        </nav>
        <div className="cta-buttons">
            <Link href=""><button className="bg-green-600 text-white font-semibold w-40 h-14 rounded-md hover:bg-black">
            Shop Now
                </button></Link>
        </div>
    </section>
